...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("# of formals and # of vals do not match")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        #print(repr(expressions)) reference
        #print(repr(expressions.second))
        ...
        env.define(target.first, LambdaProcedure(expressions.first.second, expressions.second, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    value = scheme_eval(expressions.first, env) #evaluates the very first expression of the expression list

    if scheme_falsep(value):
        return False
    elif scheme_truep(value) and expressions.second is nil:
        return value
    else:
...
def do_or_form(expressions, env):
    ...
    if scheme_truep(value):
        return value
    elif scheme_falsep(value) and expressions.second is nil:
        return False
    else:
...
def do_cond_form(expressions, env):
    ...
    print(expressions.first.first)"""
    #print(repr(expressions.first.second))
...
def make_let_frame(bindings, env):
    ...
    def make_pair(name, binded, parameter):
        nonlocal parameters
        nonlocal values
        if name == 'parameter':
            parameters = Pair(binded, parameter)
        if name == 'value':
            values = Pair(binded, values)

    while current_binding != nil:
        check_form(current_binding.first, MIN_BIND_LENGTH, MAX_BIND_LENGTH)
        make_pair('parameter', current_binding.first.first, parameters)
        make_pair('value', scheme_eval(current_binding.first.second.first, env), values)
        current_binding = current_binding.second
